 ...
 * Отримання калорійності продукту: Метод getProductCalories повертає калорійність продукту за його назвою. Якщо продукт
 * не знайдено, повертає рядок 'Product not found'.
...
class CalorieCalculator {
  ...
  addProduct(productName, calories) {
    this.productsList.set(productName, calories);
  }

  getProductCalories(productName) {
    return this.productsList.get(productName) || 'Product not found';
  }

  removeProduct(productName) {
    this.productsList.delete(productName);
  }
}
 ...
 * Додавання імен користувачів: Метод addUser дозволяє додати нове ім'я до набору. Якщо ім'я вже існує, воно не буде
 * додано повторно, зберігаючи унікальність імен у наборі.
...
class UniqueUsernames {
  ...
  exists(username) {
    return this.userNamesList.has(username);
  }
