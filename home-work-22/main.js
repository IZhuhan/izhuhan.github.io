...
const myPow = (a, b, printFunc) => {
  const internalPow = (c, d) => {
    if (d === 0) return 1;
    if (d > 0) return c * internalPow(c, d - 1);
    if (d < 0) return 1/a * internalPow(c, d + 1);
  }

  const result = internalPow(a, b);
  return printFunc(a, b, result);
};

console.log(myPow(3, 4, myPrint)) // 3^4=81
console.log(myPow(2, 3, myPrint)) // 2^3=8
console.log(myPow(2, 0, myPrint)) // 2^0=1
console.log(myPow(2, -3, myPrint)) // 2^-2=0.25
...
const myMax = (list) => Math.max.apply(null, list);
