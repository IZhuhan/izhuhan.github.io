 ...
 * додати новий елемент. Функція створює новий елемент з вказаним тегом та текстовим вмістом і додає цей елемент до заданого контейнера.
 *
 * Вимоги:
...
console.dir(createDomElement('p', 'This paragraph has been added to the specified container.', container));
...
function setUserInfoCookie(key, value) {
  const encodedCookieValue = `${encodeURIComponent(key)}=${encodeURIComponent(value)}`;
  document.cookie = `userInfo=${encodedCookieValue}; max-age=10; path=/`;

  if (document.cookie.includes(encodedCookieValue)) {
    console.log(`Cookie "${encodedCookieValue}" successfully set!`);
  }
}

// Демонстрація використання функції
setUserInfoCookie('userName', 'John Doe');
 ...
 * зберігати інформацію про користувача, а друга - отримувати її. Крім того, обидві функції повинні виводити відповідні
 * повідомлення у консоль про успішне збереження або отримання даних.
 *
...
function getUserInfo(key) {
  ...
  console.log(`Retrieved ${key}: ${sessionStorageItem}`);
